...
{
	gStandardKline.valid.back() = false;
	gStandardKline.high.emplace_back(srcRight.high);
	gStandardKline.low.emplace_back(srcRight.low);
	gStandardKline.valid.emplace_back(true);
}
...
static void CombineUpKlines(const Kline& standardLeft, const Kline& srcRight)
{
	...
	//gStandardKline.high.back() = tempLine.high;
	//gStandardKline.low.back() = tempLine.low;
	gStandardKline.valid.back() = false;

	gStandardKline.high.emplace_back(tempLine.high);
	gStandardKline.low.emplace_back(tempLine.low);
	gStandardKline.valid.emplace_back(true);
}
...
static void KlineStandardise(int lineNum, float* output, float* high, float* low, float* date)
{
	...
	for (int i = 0; i < lineNum; i++) {
		if (i == 0) {
			gStandardKline.high.emplace_back(high[i]);
			gStandardKline.low.emplace_back(low[i]);
			gStandardKline.valid.emplace_back(true);
			continue;
		}
		Kline leftKline = {
			.high = gStandardKline.high.back(),
			.low = gStandardKline.low.back(),
			.valid = gStandardKline.valid.back(),
		};
		Kline rightKline = {
			.high = high[i],
			.low = low[i],
...
void KlineGetHighSet(int lineNum, float* output, float* high, float* low, float* date)
{
	...
	for (auto i = 0; i < lineNum; i++) {
		output[i] = gStandardKline.high[i];
	}
	gStandardKline.high.clear();
	gStandardKline.low.clear();
	gStandardKline.valid.clear();
}
...
void KlineGetValidSet(int lineNum, float* output, float* high, float* low, float* date)
{
	...
	KlineStandardise(lineNum, output, high, low, date);
	for (auto i = 0; i < lineNum; i++) {
		output[i] = gStandardKline.valid[i];
	}
	gStandardKline.high.clear();
	gStandardKline.low.clear();
	gStandardKline.valid.clear();
